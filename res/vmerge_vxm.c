...
void vmerge_vxm(size_t n, const uint64_t x[], const uint64_t y, const uint8_t m[], uint64_t z[]);
...
int main() {
  ...
  uint64_t y = 0x6cb1d59e55391a3b;
  uint8_t m[13] = {0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55};
  uint64_t z[100];

  vmerge_vxm(100, x, y, m, z);

  for (int i = 0; i < 100; i++) {
    if (z[i] != (get_bit(m, i) == 0 ? x[i] : y)) {
      return 1;
    }
  }
