...
void vnclip_wx(size_t n, const uint64_t x[], const uint32_t y, uint32_t z[]);
...
int main() {
  ...
  uint32_t y = 0xbafd5e87;
  uint32_t z[100];

  vnclip_wx(100, x, y, z);

  for (int i = 0; i < 100; i++) {
    int64_t ra = int_rounding_rdn((__int128)(int64_t)x[i], y & 63);
    int64_t r0 = ra > INT_MAX ? INT_MAX : ra < INT_MIN ? INT_MIN : ra;
    int64_t rb = int_rounding_rnu((__int128)(int64_t)x[i], y & 63);
    int64_t r1 = rb > INT_MAX ? INT_MAX : rb < INT_MIN ? INT_MIN : rb;
    if ((z[i] != (uint32_t)r0) && (z[i] != (uint32_t)r1)) {
      return 1;
    }
  }
