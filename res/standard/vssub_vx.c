...
void vssub_vx(size_t n, const uint64_t x[], const uint64_t y, uint64_t z[]);

int64_t saturating_sub(int64_t x, int64_t y) {
  uint64_t ux = x;
  uint64_t uy = y;
  uint64_t res = ux - uy;
  int sh = 63;
  /* Calculate overflowed result. (Don't change the sign bit of ux) */
  ux = (ux >> sh) + (((uint64_t)0x1 << sh) - 1);
...
int main() {
  ...
  for (int i = 0; i < 100; i++) {
    if (z[i] != saturating_sub((int64_t)x[i], (int64_t)y)) {
      return 1;
    }
  }
