...
uint32_t vmsbf_m(size_t n, const uint32_t x, const uint32_t m);

uint32_t sbf(const uint32_t n, const uint32_t m) {
  uint32_t c = 0;
  uint32_t found_first_bit = 0;
  for (int i = 0; i < 32; i++) {
    uint32_t nb = (n >> i) & 1;
    uint32_t mb = (m >> i) & 1;
    if (mb == 0) {
