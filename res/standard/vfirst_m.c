...
uint64_t vfirst_m(size_t n, const uint32_t x, const uint32_t m);

uint64_t first(uint32_t n) {
  uint64_t count = 0;
  while ((n & 1) == 0) {
    n = n >> 1;
    count++;
...
int main() {
  ...
  uint32_t m[100] = {0x37ab37da, 0x2d48e7b2, 0xf50347e2, 0x589ac5ff, 0x48ab9061, 0xce17fd16, 0xad21ca7e, 0x76ce0adb,
  ...
  for (int i = 0; i < 100; i++) {
    uint64_t y = vfirst_m(32, x[i], m[i]);
    if (y != first(x[i] & m[i])) {
      return 1;
    }
  }
