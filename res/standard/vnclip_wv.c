...
void vnclip_wv(size_t n, const uint64_t x[], const uint32_t y[], uint32_t z[]);
...
int main() {
  ...
  for (int i = 78; i < 100; i++) {
    int64_t ra = int_rounding_rdn((__int128)(int64_t)x[i], y[i] & 63);
    int64_t r0 = ra > INT_MAX ? INT_MAX : ra < INT_MIN ? INT_MIN : ra;
    int64_t rb = int_rounding_rnu((__int128)(int64_t)x[i], y[i] & 63);
