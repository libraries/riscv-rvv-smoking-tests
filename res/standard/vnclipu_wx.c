...
void vnclipu_wx(size_t n, const uint64_t x[], const uint32_t y, uint32_t z[]);
...
int main() {
  ...
  vnclipu_wx(100, x, y, z);

  for (int i = 0; i < 100; i++) {
    uint64_t ra = int_rounding_rdn(x[i], y & 63);
    uint32_t r0 = ra > 0xffffffff ? 0xffffffff : (uint32_t)ra;
    uint64_t rb = int_rounding_rnu(x[i], y & 63);
